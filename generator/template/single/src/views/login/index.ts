...
export default class Login extends Vue {
    ...
    public onLogin(): void {
        ...
        this.isLoading = true;
        this.loginService.login(this.form.username, this.form.password).then(res => {
    ...
    public doLoginError(error: any) {
        ...
        if (count >= 5 || error.response.data.error_description.indexOf("账号已经被锁定") !== -1) {
            this.$Message.error(
                "错误次数到达5次,账号已被锁定,请十分钟后再试"
            );
        } else {
            this.$Message.error("账户或密码不正确,错误次数达到5次,账号将被锁定,剩余次数:" + (5 - count)); // 密码不正确
        }
        return;
    }
